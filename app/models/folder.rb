...
class Folder < ActiveRecord::Base
  ...
  def self.create_folders!
    ...
    formatted_posts.each do |post|
      should_create_folder = existing_folders.none? { |f| f.name == post['message']['hash_tag'] }
      Folder.create(name: post['message']['hash_tag'], description: "new folder") if should_create_folder     
    end   
  end
end